...
- Start without a role; when context reveals a perspective gap, add a concise **Role block** before the task directions so the agent anchors on the needed lens
...
### Dynamic Role Prompt Composer

When a consultation needs a clear vantage point (e.g., "observability-pragmatic SRE" or "principal front-end accessibility reviewer"), derive a role prompt instead of reaching for a pre-built specialist:
- **Collect signals** during discovery: What domain, lifecycle phase, or risk surfaced?
- **Name the role** with two parts: seniority/expertise + domain focus (e.g., "Staff Infrastructure Resiliency Engineer").
- **Define mandate** in one sentence: highlight priorities / trade-offs to enforce.
- **Slot role into prompt** directly above the instructions:
  ```
  Role: Staff Infrastructure Resiliency Engineer focused on failure-mode analysis for regional outages.
  Task: Review...
  ```
- If three iterations still leave uncertainty, escalate back to planning rather than over-constraining the role.

Keep the role scoped to the consultation. When dispatching another agent later, recompute the role based on fresh context instead of recycling the old one.
...
**Use general-purpose or domain-focused agents when you need:**
...
## Task Prompt Iteration Protocol (max 3 iterations)
...
- Decide whether the agent needs a **Role block**. If yes, insert it above the task description using the Dynamic Role Prompt Composer checklist.

2. **Present the proposed task prompt to the agent you chose to consult with for feedback**:
   ```
   "I'm planning to task you with: [task description]

   What additional information or context would help ensure you can successfully the task?"
   ```
...
4. **Update task prompt** with requested info. When the agent asks for a specific vantage point, revisit the Role block and sharpen it (seniority + domain + mandate).
