...
1. **Abort** with `git rebase --abort` to restore the original state
2. **Report** the conflict to the user — do not attempt to resolve automatically
3. The fixup commit still exists and can be retried after the user resolves the underlying issue
